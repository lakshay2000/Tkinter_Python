...
def change():
    api_request = requests.get("https://www.airnowapi.org/aq/observation/zipCode/current/?format=application/json&zipCode=" +
                               zip.get() + "&distance=5&API_KEY=988009DA-FF7A-41EE-A559-A7E3C22DC51C")
    api = json.loads(api_request.content)
    city = api[0]['ReportingArea']
    quality = api[0]['AQI']
    category = api[0]['Category']['Name']

    myLabel.config(text=city + " Air Quality " + str(quality) + " " +
                   category, font=('Helvectica', 20), background=messages(category))
